...
#[Question]
#used in pre-train.py
def gen_ran_output(data, model):
    vice_model = deepcopy(model)
    for (vice_name, vice_model_param), (name, param) in zip(vice_model.name_parameters(), model.named_parameters()):
...
#used in pre-trian.py
def load_data4pretrian(dataname='CiteSeer', num_parts=200):
...
def permute_edges(data, aug_ratio):
    """
    only change edge_index, all the other keys unchanges and consistent
    """
    node_num, _ = data.x.size()
    _, edge_num = data.edge_index.size()
