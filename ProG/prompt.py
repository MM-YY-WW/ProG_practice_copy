...
class GNN(torch.nn.Module):
    def __init__(self, input_dim, hid_dim=None, out_dim=None, gcn_layer_num=2, pool=None, gnn_type='GAT'):
        ...
        if hid_dim is None:
            hid_dim = int(0.618 * input_dim) #2333 golden cut 
        if out_dim is None:
            out_dim = hid_dim
        if gcn_layer_num < 2:
            raise ValueError('GNN layer_num should >=2 but you set {}'.format(gcn_layer_num))
        elif gcn_layer_num == 2:
            self.conv_layers = torch.nn.ModuleList([GraphConv(input_dim, out_dim), GraphConv(hid_dim, out_dim)])
        else:
...
class LightPrompt(torch.nn.Module):
    def __init__(self, token_dim, token_num_per_group, group_num=1, inner_prune=None):
        ...
        :param isolate_tokens: if True, then inner tokens have no connection.
...
class HeavyPrompt(LightPrompt):
    def __init__(self, token_dim, token_num, cross_prune=0.1, inner_prune=0.01):
        super(HeavyPrompt, self).__init__(token_dim, token_num, 1, inner_prune) # only has one prompt grpah
        self.cross_prune = cross_prune
    
    def forward(self, graph_batch: Batch):
        """
        TODO: although it receieves fraph batch, currently they only implement one-by-one computing instread of batch computing
        TODO: they will implement batch computing one they figure out the memory sharing mechanism within PyG
        :param graph_batch:
        :return: 
        """
...
class Pipeline(torch.nn.Module):
    def __init__(self, input_dim, dataname, gcn_layer_num=2, hid_dim =16, num_classes=2, task_type="multi_label_calssification",
