...
def meta_test_adam(meta_test_task_id_list, dataname, K_shot, seed, maml, adapt_steps_meta_test, lossfn):
    ...
    for task_1, task_2, support, query, _ in load_tasks('test', task_pairs, dataname, K_shot, seed):
        ...
        #lambda expression is anoymous function. before : is the parameters, after : is return values
        #the value of p is from model.parameters(), therefore lambda p : p.requires_grad will only return the
        #values in model.parameters() which param.requires_grad = True to filter out the parameters do not require gradients.
        ...
        for _ in range(adapt_steps_meta_test):
            ...
            for batch_id, support_batch in enumerate(support_loader):
                ...
                print("{}/{} training loss: {:.8f}".format(batch_id+1, len(support_loader), support_loss.item()))

                if batch_id == len(support_loader) - 1:
                    last_loss = running_loss / len(support_loader) #to calculate loss per batch
                    print('{}/{} training loadd: {:.8f}'.format(_, adapt_steps_meta_test, last_loss))
                    running_loss = 0.

            test_model.eval()
            #torchmetrics could define a metric function that have expect evaluation metrics
            ...
            acc = metric.compute()
            print("""\ttask pari({}, {}) | Acc: {:.4} """.format(task_1, task_2, acc))
            metric.reset()        
...
def meta_train_maml(epoch, maml, lossfn, opt, meta_train_task_id_list, dataname, adapt_steps, K_shot = 100):
    if len(meta_train_task_id_list) < 2:
        raise AttributeError("\ttask_id_list should contrian at least two tasks!")
...
def model_components():
    ...
    adapt_lr = 0.01
    meta_lr = 0.001
    model = Pipeline(input_dim=100, dataname='CiteSeer', gcn_layer_num=2, hid_dim = 100, num_classes=2,
                     task_type = "multi_label_classification",
                     token_num = 10, cross_prune=0.1, inner_prune=0.3, gnn_type='TransformerConv')
