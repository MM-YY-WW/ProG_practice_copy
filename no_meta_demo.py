...
from ProG.utils import mkdir, load_data4pretrian
from ProG.prompt import GNN, LightPrompt, HeavyPrompt
from torch import nn, optim
from ProG.data import multi_class_NIG
import torch
from torch_geometric.loader import DataLoader
from ProG.eval import acc_f1_over_batches

# this file cannot move in ProG.utils.py because it willl cause self-loop import
def model_create(dataname, gnn_type, num_class, task_type="multi_class_classification", tune_answer=False, device = torch.device("cpu")):
    if task_type in ['multi_class_classification', 'regression']:
        input_dim, hid_dim = 100, 100
        lr, wd = 0.001, 0.0001
        tnpc = 100 # token number per class

        #load pre_trained GNN
        ...
        if tune_answer:
            ...
            opi_answer = optim.Adam(filter(lambda p: p.requires_grad, answering.parameters()), lr = 0.01, weight_decay = 0.00001)
        else:
            answering, opt_answer = None, None
        gnn.to(device)
        PG.to(device)
...
def pretrain():
    ...
    print("load data...")
    graph_list, input_dim, hid_dim = load_data4pretrian(dataname, num_parts)
    ...
    pt.train(dataname, graph_list, batch_size=batch_size, 
             aug1 = 'dropN', aug = 'permE', aug_ratio=None,
             lr = 0.01, decay=0.0001, epochs=100)

def prompt_w_o_h(dataname='CiteSeer', gnn_type="TransformerConv", num_class=6, task_type = "multi_class_classificaiton",
    ...
    for j in range(1, prompt_epoch +1):
        ...
        for batch_id, train_batch in enumerate(train_loader):
            ...
            if task_type == 'multi_class_classsification':
            ...
            if batch_id % 5 == 4: #report every 5 updates
                ...
                print(
                    'epoch {}/{} | batch {}/{} | loss: {:.8f}'.format(j, prompt_epoch, batch_id+1, len(train_loader), last_loss)
                )
                running_loss = 0.
...
if __name__ == '__main__':
    print("PyTorc version:", torch._version__)
    ...
    prompt_w_o_h(dataname="CiteSeer", gnn_type='TransformerConv', num_class=6, task_type="multi_class_classification",
                 device = device)
    # prompt_w_h(dataname="CiteSeer", gnn_type="TransformerConv", num_class=6, task_type="multi_class_classifiation", device = device)